...
import { Link,useLocation ,useNavigate} from 'react-router-dom'//Navigate without page reload
...
const Navbar = ({setShow}) => {
  ...
  return (
    <div className={'flex item-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 text--grey-600 border-b border-borederColor relative transition-all  ${location.pathname === "/" && "bg-light"}'}  >
...
`}>
      ...
      {menuLinks.map((link, index) => ( //item in array, index of item
...
outline-none placeholder-grey-500" placeholder="Search products"/>
